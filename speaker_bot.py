...
# LOCAL USE ONLY:
# For Railway/GitHub, leave this placeholder unchanged.
...
# Railway persistent volume path if attached; otherwise current folder.
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not is_admin(update):
        # Quietly ignore unauthorized users inside groups.
...
async def addgroup(update: Update, context: ContextTypes.DEFAULT_TYPE):
    # Only Amanda/admin can register a group.
...
async def broadcast(update: Update, context: ContextTypes.DEFAULT_TYPE):
    # IMPORTANT:
    # Ignore ALL normal messages inside Telegram groups.
    # Group members can chat normally and BUK Speaker stays silent.
    if update.effective_chat.type != "private":
        return

    # Only the authorized administrator can broadcast privately.
    if not is_admin(update):
        await update.message.reply_text(
            "⛔ You are not authorized to use this bot."
        )
